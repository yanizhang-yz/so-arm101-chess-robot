...
class Op:
    ...
    piece: str | None = None   # promotion piece symbol for place_spare, e.g. "Q"
...
def plan_move(board: chess.Board, move: chess.Move) -> list[Op]:
    ...
    if board.is_en_passant(move):
        ...
        ops.append(Op("remove", src=chess.square_name(cap_sq)))
    elif board.is_capture(move):
        ops.append(Op("remove", src=dst))

    # 2. Move the piece (or, for promotion, retire the pawn + drop in a spare).
    if move.promotion:
        ops.append(Op("remove", src=src))  # retire the pawn off-board
        ops.append(Op("place_spare", dst=dst, piece=chess.piece_symbol(move.promotion).upper()))
    else:
        ops.append(Op("carry", src=src, dst=dst))
    ...
    if board.is_castling(move):
        ...
        ops.append(Op("carry", src=chess.square_name(rook_from), dst=chess.square_name(rook_to)))
